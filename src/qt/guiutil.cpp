...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("zilbercoin"))
        return false;
...
{
    // Convert zilbercoin:// to zilbercoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("zilbercoin://"))
    {
        uri.replace(0, 12, "zilbercoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Zilbercoin.lnk";
}
...
{
    return GetAutostartDir() / "zilbercoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Zilbercoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  zilbercoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Zilbercoin-Qt"));
